...
export default function Pricing({ data }) {
  ...
  return (
    <section className="w-full py-12 pb-18 px-6 relative overflow-hidden">
      {/* Gradient background */}
      <div className="absolute inset-0 bg-gradient-to-b  via-[#0d0215] to-black opacity-90" />

      <div className="relative z-10 max-w-7xl mx-auto text-center">
        {/* Heading */}
        <h2 className="heading-3d no-underline">{title}</h2>

        <p className=" text-white max-w-2xl mx-auto text-base md:text-lg lg:text-xl">
          {subheading}
        </p>
...
function PricingCard({ title, price, features, popular, setOpenContact }) {
  return (
    <div
      ...
      bg-gradient-to-b from-[#6a1bdc] to-[#1a1a1a] group
      `}
    >
      {/* Popular badge */}
      {popular && (
        <div
          className="absolute -top-5 left-1/2 -translate-x-1/2 
                        bg-white text-[#6E00FF] text-sm font-bold 
      ...
      <div className="py-8 text-center">
        ...
        <h3 className="mt-6 text-white">
          <span className="text-sm font-medium mr-1">starts at</span>
    ...
    group-hover:shadow-[0_0_24px_rgba(255,255,255,0.7)]
